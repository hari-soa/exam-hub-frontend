...
export default function AdminResults() {
  ...
  const { data: questionsData, loading: questionsLoading } = useFetch(
  ...
  const filteredResults = useMemo(() => {
    ...
    return sourceResults.filter((result) =>
      `${result.student || result.studentName} ${result.id || result.studentId}`
        .toLowerCase()
        .includes(query),
    );
  ...
  const average = sourceResults.length
    ? sourceResults.reduce(
        (acc, item) => acc + (item.note ?? item.score ?? 0),
        0,
      ) / sourceResults.length
    : 0;

  const passedCount = sourceResults.filter(
    (item) => (item.note ?? item.score ?? 0) >= 10,
  ).length;
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="grid gap-4 lg:grid-cols-[1.3fr_0.7fr]">
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
            ...
            <p className="mt-3 text-sm text-slate-600">
              {exam.locked
                ? "Verrouillé - tentatives enregistrées"
                : "Édition disponible"}
            </p>
          </div>
        </div>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
            ...
            <p className="mt-2 text-sm font-semibold text-slate-800">
              {exam.endAt
                ? new Date(exam.endAt).toLocaleString("fr-FR", {
                    dateStyle: "medium",
                    timeStyle: "short",
                  })
                : "N/A"}
            ...
            <div className="flex items-center justify-between border-b border-slate-200 p-5">
              <div>
                ...
                <h3 className="mt-1 text-xl font-semibold text-slate-800">
                  {selectedResult.student || selectedResult.studentName}
                </h3>
              </div>
              <button
                type="button"
            ...
            <div className="flex-1 overflow-y-auto p-5">
              <div className="mb-5 grid gap-3 sm:grid-cols-3">
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                  ...
                  <p className="mt-2 text-sm font-semibold text-slate-800">
                    {selectedResult.id || selectedResult.studentId}
                  </p>
                </div>
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                  <p className="text-[10px] uppercase tracking-[0.14em] text-slate-400">
                  ...
                  <p className="mt-2 text-sm font-semibold text-slate-800">
                    {selectedResult.note ?? selectedResult.score}/20
                  </p>
                </div>
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                  <p className="text-[10px] uppercase tracking-[0.14em] text-slate-400">
              ...
              <div className="space-y-4">
                {questions.map((question, index) => {
                  const answer = selectedResult.responses?.find(
                    (entry) =>
                      entry.questionId === question.id ||
                      entry.id === question.id,
                  );
                  const selectedChoiceId =
                    answer?.selectedChoiceId ?? answer?.selectedIndex;
                  ...
                  return (
                    ...
                    >
                      <div className="mb-3 flex items-center justify-between gap-3">
                        ...
                        <span
                          className={`inline-flex rounded-full px-2.5 py-1 text-[10px] font-semibold ${isCorrect ? "bg-emerald-100 text-emerald-700" : "bg-rose-100 text-rose-700"}`}
                        >
                          ...
                          (choice, optionIndex) => {
                            ...
                            return (
                              <div
                                ...
                                className={`rounded-xl border p-2.5 text-sm ${tone}`}
                              >
