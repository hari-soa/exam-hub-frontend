...
export default function AdminResults() {
  ...
  const sourceResults = useMemo(() => resultsData?.results || resultsData || [], [resultsData]);
  const questions = useMemo(() => questionsData?.questions || questionsData || [], [questionsData]);
  ...
  const handleExportCsv = () => {
    const rows = [
      ['Student', 'ID', 'Attempts', 'Average', 'Score'],
  ...
  if (examError || !exam) {
    return (
        <div className="rounded-2xl border border-rose-200 bg-rose-50 p-6 text-rose-700">
          Exam not found or error loading data.
        </div>
    );
  }
  ...
  return (
      <div className="space-y-6">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            <p className="text-sm font-medium text-blue-600">Results</p>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="grid gap-4 lg:grid-cols-[1.3fr_0.7fr]">
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs font-medium uppercase tracking-[0.12em] text-slate-500">Course Code</span>
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <div className="flex items-center justify-between gap-2">
                <span className="text-xs font-medium uppercase tracking-[0.12em] text-slate-500">Status</span>
                <span className={`inline-flex rounded-full px-2.5 py-1 text-[10px] font-semibold ${STATUS_STYLES[exam.status] || 'bg-slate-100 text-slate-700'}`}>
                {STATUS_LABELS[exam.status] || exam.status}
              </span>
              </div>
              <p className="mt-3 text-sm text-slate-600">
                {exam.locked ? 'Locked - submissions recorded' : 'Editing available'}
              </p>
            </div>
          </div>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <p className="text-xs font-medium uppercase tracking-[0.12em] text-slate-500">Start Date</p>
              <p className="mt-2 text-sm font-semibold text-slate-800">
                {exam.startAt ? new Date(exam.startAt).toLocaleString('en-US', { dateStyle: 'medium', timeStyle: 'short' }) : 'N/A'}
              </p>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <p className="text-xs font-medium uppercase tracking-[0.12em] text-slate-500">End Date</p>
              <p className="mt-2 text-sm font-semibold text-slate-800">
                {exam.endAt ? new Date(exam.endAt).toLocaleString('en-US', { dateStyle: 'medium', timeStyle: 'short' }) : 'N/A'}
              </p>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <p className="text-xs font-medium uppercase tracking-[0.12em] text-slate-500">Duration</p>
              <p className="mt-2 text-sm font-semibold text-slate-800">{exam.duration ? `${exam.duration} mins` : 'N/A'}</p>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <p className="text-sm text-slate-500">Passed (&ge; 10/20)</p>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex items-center gap-2 text-sm text-slate-500">
              ...
              {filteredResults.length} student(s)
            </div>
          </div>
          ...
          <div className="overflow-hidden rounded-xl border border-slate-200">
            <table className="min-w-full divide-y divide-slate-200 text-left">
              ...
              {filteredResults.length === 0 && (
                  <tr>
                    <td colSpan="6" className="px-4 py-6 text-center text-sm text-slate-500">No results found.</td>
                  </tr>
              )}
              </tbody>
            </table>
            ...
            <div className="fixed inset-0 z-[100] bg-slate-900/40 backdrop-blur-sm">
              ...
              <aside className="absolute inset-y-0 right-0 flex w-full max-w-2xl flex-col bg-white shadow-2xl">
                <div className="flex items-center justify-between border-b border-slate-200 p-5">
                  <div>
                    <p className="text-xs font-medium uppercase tracking-[0.12em] text-slate-500">Attempt</p>
                ...
                <div className="flex-1 overflow-y-auto p-5">
                  <div className="mb-5 grid gap-3 sm:grid-cols-3">
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                      <p className="text-[10px] uppercase tracking-[0.14em] text-slate-400">Attempts</p>
                      <p className="mt-2 text-sm font-semibold text-slate-800">{selectedResult.attempts || 1}</p>
                    </div>
                  </div>

                  <div className="mb-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                    <p className="text-xs font-medium uppercase tracking-[0.12em] text-slate-500">Submission Date</p>
                    <p className="mt-2 text-sm text-slate-700">
                      {selectedResult.submittedAt
                          ? new Date(selectedResult.submittedAt).toLocaleString('en-US', { dateStyle: 'medium', timeStyle: 'short' })
                          : 'N/A'}
                    </p>
                  </div>
                              ...
                              {(question.choices || question.options || []).map((choice, optionIndex) => {
                                ...
                                return (
                                    <div key={choice.id || optionIndex} className={`rounded-xl border p-2.5 text-sm ${tone}`}>
                                      ...
                                      {isSelected && <span className="ml-2 text-[10px] uppercase tracking-[0.12em] font-semibold">(Student Choice)</span>}
                                      {isAnswer && <span className="ml-2 text-[10px] uppercase tracking-[0.12em] font-semibold">(Correct Answer)</span>}
                                    </div>
                                );
                              })}
                            </div>
