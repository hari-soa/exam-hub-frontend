    ...
    return (
        <div className="p-8">
          <div className="rounded-2xl border border-red-200 bg-red-50 p-6 text-red-700">
            Error loading courses from the server.
          </div>
        </div>
    );
  }
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <h2 className="mt-1 text-3xl font-bold text-slate-800">Course Management</h2>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    ...
                    >
                      <div>
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <span className={`inline-flex rounded-full px-2.5 py-1 text-[10px] font-semibold ${STATUS_STYLES[course.status] || "bg-slate-100 text-slate-700"}`}>
                      {STATUS_LABELS[course.status] || course.status}
                        ...
                        <div className="mt-4 space-y-2 text-sm text-slate-600">
                          <div className="flex items-center justify-between">
                            <span>Professor</span>
                            <span className="font-medium text-slate-700">{course.professor || "Unassigned"}</span>
                          </div>
                          <div className="flex items-center justify-between">
                            <span>Credits</span>
          ...
          <div className="mt-4 flex items-center justify-between text-sm text-slate-500">
            <span>{filteredCourses.length} course(s) displayed</span>
            <div className="flex gap-2">
              <button type="button" className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 hover:bg-slate-50">Previous</button>
              <button type="button" className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 hover:bg-slate-50">Next</button>
            </div>
          </div>
        </div>
      </div>
