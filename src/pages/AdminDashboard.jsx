...
import useFetch from "../hooks/useFetch";// Ajustez le chemin selon l'emplacement de votre hook useFetch

const statusStyles = {
  "Désactivé": "bg-slate-100 text-slate-700",
  "Disponible": "bg-emerald-100 text-emerald-700",
  "En cours": "bg-amber-100 text-amber-700",
  // Support optionnel pour les clés en anglais renvoyées par l'API
    ...
    return (
      <div className="p-8">
        <div className="rounded-2xl border border-red-200 bg-red-50 p-6 text-red-700">
          Erreur lors du chargement des données du tableau de bord depuis le serveur.
        </div>
      </div>
    );
  }
    ...
    <div className="p-8 space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <h2 className="mt-1 text-3xl font-bold text-slate-800">Tableau de bord</h2>
        </div>
        <button
          type="button"
          onClick={() => alert("Fonctionnalité d'exportation du rapport à connecter")}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {statCards.map(({ key, label, accent, icon: Icon }) => (
          <div key={key} className="h-full flex flex-col justify-between shadow-sm hover:shadow-md transition-shadow duration-200 border border-slate-100 rounded-2xl bg-white p-5">
            <div className="flex items-center justify-between">
              ...
              <div className={`flex h-11 w-11 items-center justify-center rounded-xl ${
                ...
                accent === "violet" ? "bg-violet-100 text-violet-600" : 
                accent === "emerald" ? "bg-emerald-100 text-emerald-600" : "bg-amber-100 text-amber-600"
              }`}>
                <Icon className="h-5 w-5" />
              </div>
            </div>
            <p className="mt-4 text-xs text-slate-500">+12.4% vs. mois précédent</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.35fr_0.65fr]">
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="mb-5 flex items-center justify-between">
            <h3 className="text-lg font-semibold text-slate-800">Statistiques par UE</h3>
            <span className="text-sm text-slate-500">Taux de réussite</span>
          </div>

          <div className="space-y-4">
            {ueStats.length === 0 ? (
              <p className="text-sm text-slate-500 py-4 text-center">Aucune statistique disponible.</p>
            ) : (
              ueStats.map((ue) => (
                <div key={ue.name}>
                  ...
                  <div className="h-2.5 w-full overflow-hidden rounded-full bg-slate-100">
                    <div className="h-full rounded-full bg-gradient-to-r from-blue-500 via-sky-500 to-indigo-500" style={{ width: `${ue.successRate}%` }} />
                  </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="mb-5 flex items-center justify-between">
            <h3 className="text-lg font-semibold text-slate-800">Examens récents</h3>
            <Link
          ...
          <div className="space-y-4">
            {recentExams.length === 0 ? (
              <p className="text-sm text-slate-500 py-4 text-center">Aucun examen récent.</p>
            ) : (
              recentExams.map((exam) => (
                <div key={exam.id} className="p-4 border border-slate-100 rounded-xl flex items-center justify-between hover:bg-slate-50 transition-colors">
                  <div>
                  ...
                  <div className="flex items-center gap-3">
                    <span className={`px-2.5 py-1 text-xs rounded-full font-medium ${getStatusBadgeClass(exam.status)}`}>
                      {exam.status}
                    </span>
                    <Link className="text-sm font-medium text-blue-600 hover:text-blue-800 transition-colors" to={`/admin/exams/${exam.id}/results`}>
                      Voir le détail
                    </Link>
                  </div>
                </div>
