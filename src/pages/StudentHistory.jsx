...
export default function StudentHistory() {
    ...
    return (
        <div className="flex min-h-[400px] items-center justify-center p-8">
          <div className="flex items-center gap-3 text-slate-500">
            ...
            <span>Loading your results history...</span>
          </div>
        </div>
    );
  }
  ...
  const passed = history.filter((item) => {
    ...
    return status === "Passed" || status === "Réussi" || status === "open" || score >= 10;
  }).length;
  ...
  const latestExamTitle = latest ? (latest.exam || latest.examTitle || latest.title || "Exam") : "No exams";
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              Review your recent performance and access detailed feedback on graded exams.
            </p>
          </div>
        </div>
        ...
        <div className="grid gap-5 md:grid-cols-3">
          <div className="group relative overflow-hidden rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm transition-all duration-200 hover:shadow-md hover:-translate-y-0.5">
            ...
            <p className="mt-2 text-xs text-slate-500 flex items-center gap-1">
              ...
              Based on {totalItems} exam{totalItems > 1 ? "s" : ""}
            </p>
          </div>
          ...
          <div className="group relative overflow-hidden rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm transition-all duration-200 hover:shadow-md hover:-translate-y-0.5">
            ...
            <p className="mt-2 text-xs text-emerald-600 font-medium flex items-center gap-1">
              ...
              {totalItems > 0 ? Math.round((passed / totalItems) * 100) : 0}% pass rate
            </p>
          </div>
          ...
          <div className="overflow-x-auto">
            {totalItems === 0 ? (
                <div className="flex flex-col items-center justify-center p-12 text-center">
                  ...
                  <p className="text-base font-semibold text-slate-700">No results available</p>
                  <p className="text-sm text-slate-400 max-w-sm mt-1">
                    You haven't taken any exams yet or your grades haven't been published.
                  </p>
                </div>
            ) : (
                <table className="min-w-full divide-y divide-slate-100 text-left">
                  ...
                  {history.map((result) => {
                    ...
                    const title = result.exam || result.examTitle || result.title || "Exam";
                    const date = result.date || (result.attemptedAt ? new Date(result.attemptedAt).toLocaleDateString("en-US") : "N/A");
                    const score = result.score ?? result.note ?? 0;
                    const total = result.total ?? 20;

                    let status = result.status;
                    if (!status || status === "Réussi") {
                      status = score >= 10 ? "Passed" : "Needs Improvement";
                    } else if (status === "À renforcer") {
                        ...
                        >
                          <span
                              className={`h-1.5 w-1.5 rounded-full ${
                                  status === "Passed" ? "bg-emerald-500" : "bg-amber-500"
                              }`}
                          />
                          {status}
                        </span>
                          ...
                          <td className="px-6 py-4 text-right whitespace-nowrap">
                            <Link
                                ...
                                className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-xs font-semibold text-slate-700 shadow-sm transition-all duration-150 hover:border-blue-300 hover:bg-blue-600 hover:text-white hover:shadow cursor-pointer active:scale-95"
                            >
